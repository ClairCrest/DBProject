...
if (isset($_POST['signup'])) {
    ...
    try {
        ...
        // Insert data into address1 table
        $stmt_address = $conn->prepare("INSERT INTO address1 (country, province, zip_code) VALUES (:country, :province, :zip_code)");
        $stmt_address->bindParam(":country", $country);
        $stmt_address->bindParam(":province", $province);
        $stmt_address->bindParam(":zip_code", $zip_code);
        ...
        $stmt_users = $conn->prepare("INSERT INTO users (balance, email, password, role_id, detail_id) VALUES (:balance, :email, :password, :role_id, :detail_id)");
        $stmt_users->bindParam(":balance", $balance);
        $stmt_users->bindParam(":email", $email);
        $stmt_users->bindParam(":password", $passwordHash);
        $stmt_users->bindParam(":role_id", $urole);
